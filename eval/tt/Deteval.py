...
input_dir = os.path.join(project_root, 'outputs/submit_r18_tt/')
gt_dir = os.path.join(project_root, 'data/TotalText/test/GT/')
fid_path = os.path.join(project_root, 'outputs/res_tt.txt')
...
def detection_filtering(detections, groundtruths, threshold=0.5):
    for gt_id, gt in enumerate(groundtruths):
        if (gt[5] == '#') and (gt[1].shape[1] > 1):
            gt_x = np.array(list(map(int, np.squeeze(gt[1])))).astype(np.int32)
            gt_y = np.array(list(map(int, np.squeeze(gt[3])))).astype(np.int32)

            gt_p = np.concatenate((gt_x, gt_y))
            ...
            for det_id, detection in enumerate(detections):
                ...
                detection = np.array(detection).astype(np.int32)
                det_y = detection[0::2]
                det_x = detection[1::2]
        ...
        for gt_id, gt in enumerate(groundtruths):
            if len(detections) > 0:
                for det_id, detection in enumerate(detections):
                    ...
                    # print(np.array(list(gt_x)).shape,np.array(list(gt_y)).shape)
                    # can't excute np.array(map()),  should be np.array(list(map()))
                    gt_p = np.concatenate((gt_x, gt_y))
                    gt_p = gt_p.reshape(2, -1).transpose()
                    gt_p = plg.Polygon(gt_p)
