...
class R18_CTW(R18_BASE):

    # 训练条件参数设置           
    cfg_name='r18_ctw'  # 配置文件名称
    dataset_name='CTW1500'  # 数据集简称  
    ...
    data = DottedDict(dict(
        train=dict(
            ...
            is_ignore=False,  # 训练数据生成过程中，该数据集是否包含 ignore 样本
        ...
        test=dict(
            ...
            is_ignore=False,  # 测试数据生成过程中，该数据集是否包含 ignore 样本
    ...
    test_cfg = DottedDict(dict(
        min_score=0.85,  # 推理过程，热力图最小平均置信度
