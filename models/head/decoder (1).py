...
class Decoder(nn.Module):
    ...
    def get_results2(self, features, cates, kernels, imgs_meta, test_cfg):
        ...
        # print(time.time()-s)
        # s = time.time()
        masks = np.zeros(imgs_meta['ori_size'].numpy()[0].tolist())
        for instance in seg_masks:
            ins = instance.cpu().numpy().astype(np.uint8)
            masks+=ins
            contours_poly, _ = cv2.findContours(ins, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        # s = time.time()
        # print("------------------------------------------------------")
        cv2.imwrite('mask/{}'.format(imgs_meta['img_path'][0].split("/")[-1]),masks*255)
        return {'bboxes': bboxes}
